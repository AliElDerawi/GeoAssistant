...
object AppSharedMethods {
    ...
    fun getCurrentUserId(): String = getSharedPreference().getString(AppSharedData.PREF_USER_ID, "")!!
    ...
    fun isForegroundAndBackgroundPermissionGranted(context: Context): Boolean =
        isForegroundPermissionGranted(context) && isBackgroundPermissionGranted(context)
...
//         Return True if user denied , and false if a user select never show again
    ...
    fun isSupportsAndroid33(): Boolean = Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU

    fun isLogin(): Boolean = getSharedPreference().getBoolean(AppSharedData.PREF_IS_LOGIN, false)
    ...
    fun GoogleMap.moveCameraToLocation(
        latLng: LatLng,
        zoom: Float,
    ) {
    ...
    fun GoogleMap.addMarkerWithName(
        latLng: LatLng,
        name: String,
    ): Marker? =
        addMarker(
            ...
            window.decorView.apply {
                setOnApplyWindowInsetsListener { v, insets ->
                    ...
                    val statusBarBackground =
                        View(this@setStatusBarColorAndStyle).apply {
    ...
    fun Context.getCompatColor(color: Int): Int = ResourcesCompat.getColor(resources, color, null)
